 ...
 * 获取所有城市列表（扁平化）
 */
export function getAllCities(): { province: string; city: string; code: string }[] {
  const result: { province: string; city: string; code: string }[] = [];
  provinces.forEach(p => {
    p.cities.forEach(c => {
      result.push({
        province: p.name,
 ...
 * 根据城市代码获取城市信息
 */
export function getCityByCode(code: string): { province: string; city: string } | null {
  for (const p of provinces) {
    const city = p.cities.find(c => c.code === code);
    if (city) {
      return { province: p.name, city: city.name };
    }
  }
  return null;
}
