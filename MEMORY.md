...
**问题描述**：
分享链接发到微信群/好友时，只显示链接文本，不显示卡片形式（带图片、标题、描述）。

**已尝试方案**：
1. ✅ 配置 Open Graph meta 标签（og:title, og:description, og:image）
2. ✅ 使用对象存储公开URL作为OG图片
3. ❌ 微信仍不显示卡片效果

**原因分析**：
- 微信对未配置JS安全域名的外部链接，可能不抓取OG信息
- 部分域名被微信标记，不显示卡片预览

**解决方案**：
1. 注册微信公众号（订阅号免费）
2. 在公众号后台配置 JS 安全域名
3. 使用微信 JSSDK 自定义分享内容

**相关文件**：
- `src/app/share/[token]/layout.tsx` - OG配置
- `src/app/dashboard/base/applications/page.tsx` - 分享弹窗
- `public/og-share.png` - 分享卡片图片
...
| 日期 | 变更内容 |
|------|----------|
| 2026-03-20 | 获取《经营范围规范表述目录》官方数据：从市场监管总局PDF提取1550条经营范围规范表述 |
| 2026-03-22 | 入驻审批功能：侧边栏改名、创建审批页面（左右布局）、实现审批通过/驳回API |
| 2026-03-22 | 在入驻申请列表页添加转发按钮（草稿状态可用） |
